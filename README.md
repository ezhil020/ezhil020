<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&amp;color=0:0f0c29,50:302b63,100:24243e&amp;height=280&amp;section=header&amp;text=EZHILVENDAN%20B%20K&amp;fontSize=55&amp;fontColor=C9A7FF&amp;animation=fadeIn&amp;fontAlignY=38&amp;desc=Full%20Stack%20Developer%20%7C%20Agentic%20AI%20Engineer&amp;descAlignY=55&amp;descAlign=50" width="100%"/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&amp;weight=600&amp;size=26&amp;duration=3000&amp;pause=800&amp;color=B39DDB&amp;center=true&amp;vCenter=true&amp;width=650&amp;lines=Building+Agentic+AI+Systems;LangGraph+%2B+Multi-Agent+Orchestration;Full+Stack+Product+Engineer;Turning+Ideas+Into+Production" alt="Typing SVG" />
</a>

<br/>

![Degree](https://img.shields.io/badge/M.Sc-Software_Systems_(Integrated)-6A0DAD?style=flat-square&logo=googlescholar&logoColor=white)
![University](https://img.shields.io/badge/Coimbatore_Institute_of_Technology-4B0082?style=flat-square&logo=googleclassroom&logoColor=white)
![Location](https://img.shields.io/badge/Location-Coimbatore,_Tamil_Nadu,_India-8A2BE2?style=flat-square&logo=mapbox&logoColor=white)

<br/>

<a href="https://linkedin.com/in/ezhil-vendan">
  <img src="https://img.shields.io/badge/LinkedIn-6A0DAD?style=for-the-badge&amp;logo=linkedin&amp;logoColor=white" />
</a>
<a href="mailto:ezhilvendan13@gmail.com">
  <img src="https://img.shields.io/badge/Email-4B0082?style=for-the-badge&amp;logo=gmail&amp;logoColor=white" />
</a>
<a href="https://github.com/ezhil020">
  <img src="https://img.shields.io/badge/GitHub-8A2BE2?style=for-the-badge&amp;logo=github&amp;logoColor=white" />
</a>

<br/><br/>

![Profile Views](https://komarev.com/ghpvc/?username=ezhil020&style=for-the-badge&color=8a2be2&label=PROFILE+VIEWS)
![Followers](https://img.shields.io/github/followers/ezhil020?style=for-the-badge&color=7b2ff7&labelColor=1a1a2e&logo=github)
![Stars](https://img.shields.io/github/stars/ezhil020?style=for-the-badge&color=b39ddb&labelColor=1a1a2e&logo=github)

</div>

<br/>

---

## 🧠 About Me

<div align="center">
  <img src="https://raw.githubusercontent.com/abhisheknaiidu/abhisheknaiidu/master/code.gif" width="380"/>
</div>

<br/>

<table>
<tr>
<td width="62%" valign="top">

### 👋 Hi, I'm Ezhilvendan

I'm a software developer who likes building things end to end — from the database schema and API up to the interface someone actually clicks on. I care about the full picture: clean architecture, smooth user experience, and systems that hold up under real use, not just in a demo.

Recently a lot of my work has pulled me toward **AI-agent systems** — teaching multiple agents to coordinate on a task instead of relying on one big model to do everything. It's an extension of the same instinct that drew me to full stack development in the first place: taking something complex and turning it into a system that actually works.

Outside of that, I'm just someone who enjoys the process — sketching an idea, building it, breaking it, and making it better.

</td>
<td width="38%" valign="top">

### ⚡ Quick Facts

- 🎓 M.Sc Software Systems (Integrated), CIT
- 📍 Coimbatore, Tamil Nadu, India
- 💼 Ex-Intern @ Infinijith Technology
- 🧑‍💻 Full Stack Developer · Agentic AI & Automation

</td>
</tr>
</table>

<div align="center">

**🎯 Currently Open To:**

![Full Time](https://img.shields.io/badge/Full--Time-Software%20Development-6A0DAD?style=flat-square)
![AI](https://img.shields.io/badge/Agentic%20AI-Engineering-7B2FF7?style=flat-square)
![FullStack](https://img.shields.io/badge/Full%20Stack-Development-8A2BE2?style=flat-square)
![OpenSource](https://img.shields.io/badge/Open%20Source-Collaboration-B39DDB?style=flat-square)

</div>

<br/>

---

## 🛠️ Tech Stack

### Languages
![Python](https://skillicons.dev/icons?i=python) ![JavaScript](https://skillicons.dev/icons?i=js) ![TypeScript](https://skillicons.dev/icons?i=ts) ![C++](https://skillicons.dev/icons?i=cpp)

### Frontend
![React](https://skillicons.dev/icons?i=react) ![NextJS](https://skillicons.dev/icons?i=nextjs) ![HTML5](https://skillicons.dev/icons?i=html) ![CSS3](https://skillicons.dev/icons?i=css)

### Backend & Databases
![NodeJS](https://skillicons.dev/icons?i=nodejs) ![Express](https://skillicons.dev/icons?i=express) ![FastAPI](https://skillicons.dev/icons?i=fastapi) ![Flask](https://skillicons.dev/icons?i=flask) ![PostgreSQL](https://skillicons.dev/icons?i=postgres) ![MongoDB](https://skillicons.dev/icons?i=mongodb) ![Redis](https://skillicons.dev/icons?i=redis) ![SQLite](https://skillicons.dev/icons?i=sqlite)

### Cloud, DevOps & Tooling
![Git](https://skillicons.dev/icons?i=git) ![GitHub](https://skillicons.dev/icons?i=github) ![Docker](https://skillicons.dev/icons?i=docker)

<br/>

---

## 🧩 Core Expertise

<div align="center">

| Domain | Proficiency | Details |
|---|:---:|---|
| **Backend Engineering** | ⭐⭐⭐⭐⭐ | REST APIs and services built with FastAPI, Express.js, and Flask across multiple production-style projects |
| **Full Stack Development** | ⭐⭐⭐⭐⭐ | End-to-end apps with React / Next.js frontends wired to backend services and databases |
| **Agentic AI / Multi-Agent Systems** | ⭐⭐⭐⭐⭐ | LangGraph-based agent orchestration for conversational design generation and automated deployment pipelines |
| **Databases & Data Modeling** | ⭐⭐⭐⭐ | Schema design and querying across PostgreSQL, MongoDB, SQLite, and Redis |
| **Speech & NLU** | ⭐⭐⭐⭐ | Whisper for speech-to-text, Gemini for intent extraction and natural language understanding |
| **Applied Machine Learning** | ⭐⭐⭐⭐ | Content-based filtering, cosine similarity ranking, LLM-based structured extraction from free text |
| **Workflow Automation** | ⭐⭐⭐⭐ | Coordinated agent workflows automating end-to-end conversational IVR development lifecycle |

</div>

<br/>

---

## 🚀 Featured Projects

<details>
<summary><b>🔹 Joblink AI — Voice-First Worker Marketplace</b></summary>
<br/>

A voice-first platform connecting informal workers with clients, using speech and language models to make hiring accessible without requiring text-based interaction.

| Attribute | Details |
|---|---|
| **Stack** | Python, React, LangGraph, FastAPI, SQLite |
| **Scale** | Multi-agent pipeline for matching, negotiation & contracts |
| **Performance** | Real-time speech-to-text via Whisper |
| **Security** | Structured contract generation with validated agent outputs |
| **Impact** | Removes literacy/language barriers for informal workers |
| **Repository** | [github.com/ezhil020/joblink-ai](https://github.com/ezhil020) |

Used Whisper for speech-to-text and Gemini for NLU and intent extraction, then orchestrated LangGraph agents to independently handle worker matching, price negotiation, and contract generation. Matching relies on a multi-factor scoring algorithm weighing skill, proximity, and availability.

</details>

<details>
<summary><b>🔹 ChitHub — Digital Chit-Fund Management Platform</b></summary>
<br/>

A platform to digitize traditional chit-fund operations, giving organizers and members clear, real-time visibility into groups, auctions, and payments.

| Attribute | Details |
|---|---|
| **Stack** | Next.js, Node.js, Express.js, PostgreSQL |
| **Scale** | End-to-end flows across onboarding, auctions & payments |
| **Performance** | Real-time auction and bidding updates |
| **Security** | Structured payment tracking and role-based member access |
| **Impact** | Brings transparency to traditionally manual, trust-based operations |
| **Repository** | [github.com/ezhil020/chithub](https://github.com/ezhil020) |

Built complete flows for member onboarding, auction management, live bidding, and payment tracking, replacing informal spreadsheet/paper-based chit-fund management with a structured digital system.

</details>

<details>
<summary><b>🔹 SpendShare — Full Stack Expense Sharing Platform</b></summary>
<br/>

A full stack expense sharing platform that helps groups split bills and settle shared expenses without the usual back-and-forth confusion.

| Attribute | Details |
|---|---|
| **Stack** | React, TypeScript, FastAPI, PostgreSQL, Redis |
| **Scale** | Personal, friend, and group expense management |
| **Performance** | Redis-backed caching for fast dashboard loads |
| **Security** | Scoped access across personal vs. group expense views |
| **Impact** | Reduces payment confusion and disputes within groups |
| **Repository** | [github.com/ezhil020/spendshare](https://github.com/ezhil020) |

Designed personalized dashboards for individual users alongside shared group views, supporting personal, friend-level, and group-level expense tracking and settlement in one unified platform.

</details>

<details>
<summary><b>🔹 Orator — AI-Powered Public Speaking Coach</b></summary>
<br/>

An AI-powered platform to help users improve public speaking through real-time feedback, interactive exercises, and adaptive scoring.

| Attribute | Details |
|---|---|
| **Stack** | Streamlit, Python, Transformers |
| **Scale** | Multiple interactive exercise modes |
| **Performance** | Real-time speech-to-text and emotion analysis |
| **Security** | N/A (local session-based practice tool) |
| **Impact** | Gives learners measurable, personalized speaking feedback |
| **Repository** | [github.com/ezhil020/orator](https://github.com/ezhil020) |

Combined real-time speech-to-text, AI-generated content prompts, emotion analysis, and adaptive performance scoring with interactive exercises — Rapid Fire, Conductor, and Triple Step — including vocal energy visualization to help users track improvement over time.

</details>

<br/>

---

## 💼 Experience

### **Software Developer Intern** — Infinijith Technology
`June 2025 – November 2025`

Contributed to an agentic AI platform automating the conversational IVR development lifecycle through coordinated AI agents and workflow orchestration.

- Built LangGraph-based multi-agent workflows for conversational design generation
- Developed automated deployment pipelines to conversational AI platforms
- Collaborated on agent coordination logic to streamline the end-to-end IVR build process

`Python` `LangGraph` `Agentic AI` `Workflow Orchestration`

<br/>

---

## 📜 Certifications

**Oracle**

![Oracle Agentic AI Foundations Associate](https://img.shields.io/badge/Oracle-Agentic_AI_Foundations_Associate-F80000?style=for-the-badge&logo=oracle&logoColor=white)

**Udemy**

![Web Development Bootcamp](https://img.shields.io/badge/Udemy-Web_Development_Bootcamp-8A2BE2?style=for-the-badge&logo=udemy&logoColor=white)

<br/>

---

## 📊 GitHub Analytics

<div align="center">

<img src="https://github-stats-extended.vercel.app/api?username=ezhil020&amp;show_icons=true&amp;theme=tokyonight&amp;hide_border=true&amp;bg_color=0d1117&amp;title_color=b39ddb&amp;icon_color=8a2be2&amp;text_color=c9c9c9&amp;count_private=true" width="49%"/>
<img src="https://streak-stats.demolab.com/?user=ezhil020&amp;theme=tokyonight&amp;hide_border=true&amp;background=0d1117&amp;ring=8a2be2&amp;fire=b39ddb&amp;currStreakLabel=b39ddb" width="49%"/>

<img src="https://github-stats-extended.vercel.app/api/top-langs/?username=ezhil020&amp;layout=compact&amp;theme=tokyonight&amp;hide_border=true&amp;bg_color=0d1117&amp;title_color=b39ddb&amp;text_color=c9c9c9" width="49%"/>

</div>

<br/>

---

## 🏆 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy-winning.vercel.app/?username=ezhil020&amp;theme=discord&amp;no-frame=true&amp;column=7&amp;margin-w=10&amp;margin-h=10&amp;row=1" />

</div>

<br/>

---

## 📈 Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=ezhil020&amp;theme=react-dark&amp;hide_border=true&amp;bg_color=0d1117&amp;color=b39ddb&amp;line=8a2be2&amp;point=ffffff" width="100%"/>

</div>

<br/>

---

## 🐍 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/ezhil020/ezhil020/output/github-contribution-grid-snake-dark.svg" width="100%"/>

</div>

<br/>

---

## 🎯 Current Focus

<table>
<tr>
<td width="50%" valign="top">

### 🔭 Learning
- System design for scalable, production-grade backends
- Modern React patterns and performance optimization
- Advanced LLM orchestration and agent design

</td>
<td width="50%" valign="top">

### 🛠️ Building
- Full stack products with clean, scalable architecture
- AI-assisted tools that solve everyday practical problems

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🔬 Exploring
- Retrieval-augmented generation (RAG) and agent memory
- Better dev tooling for everyday, repetitive engineering tasks

</td>
<td width="50%" valign="top">

### 🎯 Open To
- Full-time Software Development roles
- AI / Agentic Engineering opportunities
- Open source collaboration

</td>
</tr>
</table>

<br/>

---

## 🤝 Connect With Me

<div align="center">

[![Gmail](https://img.shields.io/badge/Gmail-4B0082?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ezhilvendan13@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-6A0DAD?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/ezhil-vendan)
[![GitHub](https://img.shields.io/badge/GitHub-8A2BE2?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ezhil020)

</div>

<br/>

---

<div align="center">

*"Great engineering is invisible — it just works, at scale, under pressure."*

<img src="https://capsule-render.vercel.app/api?type=waving&amp;color=0:24243e,50:302b63,100:0f0c29&amp;height=150&amp;section=footer"/>

</div>
